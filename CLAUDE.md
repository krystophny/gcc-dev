...
The pre-push checklist below is automated by `scripts/hooks/pre-push`;
install it once with `make install-hooks`. CI runs `make check-meta` on
every PR via `.github/workflows/meta.yml` (local validation only; no
Bugzilla, GCC git, or GitHub API traffic).
