# PR79524 Maintainer Summary

- **Bugzilla:** https://gcc.gnu.org/bugzilla/show_bug.cgi?id=79524
- **GitHub issue:** https://github.com/krystophny/gcc-dev/issues/55
- **Fix status:** patch-ready
- **Regression:** no
- **Severity:** other
- **Validity class:** valid-code
- **Trunk commit:** n/a
- **Trunk patch:** 0001-fortran-Clean-up-charlens-after-rejected-parameter-a.patch

## Active Release Branch Matrix

| Branch | Reproduces | Candidate | Apply mode | Targeted tests | Full suite |
|--------|------------|-----------|------------|----------------|------------|
| gcc-13 | unknown | unknown | unknown | not-run | not-run |
| gcc-14 | unknown | unknown | unknown | not-run | not-run |
| gcc-15 | unknown | unknown | unknown | not-run | not-run |

## Risk Summary

This is a `other` fix against `valid-code`. Branch-specific patch data and test evidence live in `status.json` and the `backports/` subdirectories.
